...
class GameControl(game: Game) extends InputAdapter {
  ...
  override def keyDown(keycode: Int): Boolean = {
    ...
    if (game.player.inRange.itList.nonEmpty) {
      if (keycode == Keys.UP) {
        game.player.inRange.up()
      }
      if (keycode == Keys.DOWN) {
        game.player.inRange.down()
      }
      if (keycode == Keys.ENTER) {
        game.player.inRange.used()
      }
    } else {
      if (game.player.inInventory && game.player.inventory.nonEmpty) {
        if (keycode == Keys.UP) {
          game.player.invMenu.up()
        }
        if (keycode == Keys.DOWN) {
          game.player.invMenu.down()
        }
        if (keycode == Keys.ENTER) {
          game.player.invMenu.update()
          game.player.invMenu.used()
          val (count, item) =
            game.player.inventory(game.player.invMenu.selected)
          item.use()
          if (count > 1) {
        ...
        if (game.player.inEquip) {
          if (keycode == Keys.UP) {
            game.player.equipMenu.up()
          }
          if (keycode == Keys.DOWN) {
            game.player.equipMenu.down()
          }
          if (keycode == Keys.ENTER) {
          ...
          if (keycode == Keys.ENTER) {
            game.shopIn.foreach(shop => {
              if (
                ...
                val index = game.player.inventory.indexWhere({
                  case (count, item) =>
                    if (
                      item == shop
                        .inventory(shop.invMenu.selected)
                        ._2 && count < 40
                    ) true
                    else false
                })
